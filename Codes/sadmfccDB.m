...
if y==1
    ...
    a=struct2cell(d);
    a=a(1,3:length(a));
    nos=length(a);                           %Number Of files in the folder
    quant= 32;                              %Enter the order of Vector quantisation.(must b power of 2)

    for i=1:nos
        name=horzcat('../Samples2\Sad\',cell2mat(a(1,i)));
        [s1,fs]=wavread(name);
        ...
        temp=mfccf(13,n1,fs);
        [m,n]=size(temp);
        [vqs(i,:),vps(i,:)]=vqsplit(temp,quant);   %Vector Quantization
    end

    %Followinf Statement generates the database of feature vectors for happy
    save sadmfccDBall.mat nos vqs
